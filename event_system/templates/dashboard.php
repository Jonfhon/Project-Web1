            ...
            <form method="GET" action="dashboard" class="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="flex flex-col">
                    ...
                    <input type="text" name="keyword" value="<?= htmlspecialchars($_GET['keyword'] ?? '') ?>" placeholder="ค้นหาชื่อกิจกรรม..." 
                        class="px-4 py-3 rounded-xl text-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-300">
                </div>
                ...
                <div class="md:col-span-3 flex gap-3 mt-2">
                    <button type="submit" class="bg-white text-indigo-600 px-6 py-3 rounded-xl font-bold hover:bg-indigo-100 transition shadow-sm">
                        ค้นหา
                    </button>
                    <a href="dashboard" class="bg-indigo-700/50 text-white px-6 py-3 rounded-xl font-bold hover:bg-indigo-700 transition shadow-sm border border-indigo-400">
                        ล้างค่า
                    </a>
                </div>
            </form>
                    ...
                    <div class="bg-white rounded-2xl shadow-md overflow-hidden border border-gray-100 group">
                        ...
                        <div class="p-6">
                            ...
                            <?php if (isset($_SESSION['user_id']) && $event['organizer_id'] == $_SESSION['user_id']): ?>
                                <button disabled class="w-full bg-indigo-50 text-indigo-400 py-3 rounded-xl font-bold cursor-not-allowed border border-indigo-100">
                                    กิจกรรมของคุณ
                                </button>
                                
                            <?php elseif (!empty($event['is_registered']) && $event['is_registered'] > 0): ?>
                                <button disabled class="w-full bg-slate-200 text-slate-500 py-3 rounded-xl font-bold cursor-not-allowed border border-slate-300">
                                    ลงทะเบียนแล้ว
                                </button>
